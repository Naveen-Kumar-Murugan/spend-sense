  ...
  return (
    <div className="animate-fade-up rounded-2xl border border-line bg-white p-6 text-center shadow-card sm:p-8">
      <span
        ...
        aria-hidden
      >
        {pending ? <Clock className="h-7 w-7" /> : <CheckCircle2 className="h-7 w-7" />}
      </span>
      ...
      <div className="mt-5 space-y-3 rounded-2xl bg-surface-sunken/70 p-4 text-left">
        ...
        <Row
          ...
          value={
            <span className="inline-flex items-center gap-2">
              ...
              {CATEGORY_META[receipt.category].label} · {titleCase(receipt.subcategory)}
            </span>
          }
        />
        ...
        <Row
          label="Status"
          value={
            <Badge tone={pending ? 'warn' : 'mint'}>{pending ? 'Pending confirmation' : 'Paid'}</Badge>
          }
        />
      </div>
      ...
      {receipt.upiIntent && (
        <div className="mt-5 space-y-2.5">
          ...
          <button
            type="button"
            onClick={() => navigator.clipboard?.writeText(receipt.upiIntent?.uri ?? '')}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-ink-muted hover:text-ink"
          >
            <Copy className="h-3.5 w-3.5" aria-hidden />
            Copy payment link
          </button>
          <Button variant="secondary" className="w-full" loading={confirming} onClick={onConfirmUpi}>
            I have paid — mark as complete
          </Button>
          <p className="text-xs text-ink-faint">
            SpendSense cannot read your UPI app, so confirm here once the payment goes through.
          </p>
        </div>
      )}
      ...
      <div className="mt-6 flex flex-col gap-2.5 sm:flex-row">
        ...
        <Button asChild variant="subtle" className="flex-1">
          <Link to={`/app/transactions/${receipt.paymentId}`}>View transaction</Link>
        </Button>
      </div>
    </div>
  );
